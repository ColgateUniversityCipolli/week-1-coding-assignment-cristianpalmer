...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# Code to load aRtsy Library}
...
\hlkwd{canvas_collatz}\hldef{(}\hlkwc{colors} \hldef{=} \hlkwd{colorPalette}\hldef{(}\hlsng{"vrolik4"}\hldef{),}
               ...
               \hlkwc{angle.even} \hldef{=} \hlopt{-}\hlnum{0.05}\hldef{,}
               \hlkwc{angle.odd} \hldef{=} \hlnum{0.0145}\hldef{,}
...
\end{knitrout}
\caption{Art Circle With Grey Background}
\label{CollatzPlot3}
\end{center}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\hlkwd{library}\hldef{(aRtsy)}
\hlkwd{set.seed}\hldef{(}\hlnum{1}\hldef{)}
\hlkwd{canvas_collatz}\hldef{(}\hlkwc{colors} \hldef{=} \hlkwd{colorPalette}\hldef{(}\hlsng{"blossom"}\hldef{),}
               \hlkwc{background} \hldef{=} \hlsng{"#800000"}\hldef{,}
               \hlkwc{n} \hldef{=} \hlnum{212}\hldef{,}
               \hlkwc{angle.even} \hldef{=} \hlnum{0.99}\hldef{,}
               \hlkwc{angle.odd} \hldef{=} \hlnum{0.0276}\hldef{,}
               \hlkwc{side} \hldef{=} \hlnum{FALSE}
\hldef{)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{my_vector} \hlkwb{<-} \hlnum{0}\hlopt{:}\hlnum{10}
\hldef{my_vector}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{result_vector} \hlkwb{<-} \hlnum{2}\hlopt{^}\hldef{(my_vector} \hlopt{+} \hlnum{1}\hldef{)} \hlopt{+} \hlnum{2}\hlopt{^}\hldef{(my_vector} \hlopt{-} \hlnum{1}\hldef{)}
\hldef{result_vector}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{which}\hldef{(result_vector}\hlopt{==}\hlnum{40.0}\hldef{)}
\end{alltt}
\begin{verbatim}
## [1] 5
\end{verbatim}
...
\textbf{Solution: This approach wouldn't work in this case since the correct value for x is not a whole number. Using the vector approach, x would need to be a whole number for this approach to work.}
\end{enumerate}
\end{enumerate}
